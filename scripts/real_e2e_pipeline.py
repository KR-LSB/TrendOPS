...
def print_stage(title: str):
    if console: console.print(Panel(f"[bold white]{title}[/bold white]", style="bold blue"))
    else: print(f"\n━━━ {title} ━━━")

def print_success(msg: str):
    if console: console.print(f"[bold green]✓ {msg}[/bold green]")
    else: print(f"✓ {msg}")
...
async def stage_trigger(max_keywords: int = 10):
    ...
    try:
        ...
        # 전체 키워드 가져오기
        keywords = [{"keyword": tk.keyword, "score": tk.trend_score, "source": tk.source} 
        ...
        if console and keywords:
            ...
            for i, kw in enumerate(keywords, 1):
                ...
                if hasattr(source_str, "value"): source_str = source_str.value
...
async def stage_deduplication_and_indexing(articles: list[dict]):
    ...
    try:
        ...
        print_success(f"중복 제거: {len(articles)}건 → {len(unique_articles)}건 (신규 {len(unique_articles)}건)")

        if new_doc_ids:
            bm25 = get_bm25_index()
            added_count = bm25.add_documents(
                doc_ids=new_doc_ids,
                documents=new_docs_content,
                metadatas=new_docs_meta
            )
...
async def stage_llm_analysis(
    ...
    for i, article in enumerate(articles[:10], 1):
        ...
        context_parts.append(f"[{i}] {article['title']} ({pub_date})\n   - {article['summary'][:200]}")
    context = "\n".join(context_parts)
...
"""
    base_user_prompt = f"키워드: '{keyword}'\n\n[관련 문서]\n{context}\n\n위 문서를 분석하여 JSON으로 출력하세요."
    ...
    async with aiohttp.ClientSession() as session:
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if attempt == max_retries: print(f"   ⚠️ 분석 실패: {e}")
    ...
    if final_analysis:
        if console:
            ...
            console.print(Panel(
                ...
                title=f"📊 분석 결과: {keyword}", border_style="green"
            ))
...
async def run_real_pipeline(
    ...
    if not trend_keywords_data: return {"success": False}
