...
async def benchmark_trigger(iterations: int = 5) -> BenchmarkResult:
    ...
    async def mock_trigger():
        ...
        return [{"keyword": f"keyword_{i}", "score": 7.0 + i * 0.5} for i in range(10)]
...
async def benchmark_collector(iterations: int = 5) -> BenchmarkResult:
    ...
    async def mock_collect():
        ...
        return [{"title": f"Article {i}", "link": f"https://example.com/{i}"} for i in range(20)]
...
async def benchmark_hybrid_search(iterations: int = 5) -> BenchmarkResult:
    ...
    def mock_search():
        ...
        return [{"doc_id": i, "score": 0.9 - i * 0.05} for i in range(items_processed)]
...
def display_results_rich(console: Console, suite: BenchmarkSuite):
    ...
    for r in suite.results:
        success_style = (
            "green" if r.success_rate >= 95 else "yellow" if r.success_rate >= 80 else "red"
        )
        table.add_row(
            r.stage,
            f"{r.duration_ms:.1f}",
...
def display_results_plain(suite: BenchmarkSuite):
    ...
    for r in suite.results:
        print(
            f"{r.stage:<15} {r.duration_ms:<10.1f} {r.p50_ms:<10.1f} {r.p95_ms:<10.1f} {r.success_rate:<10.1f}%"
        )
...
def export_results(suite: BenchmarkSuite, format: str):
    ...
    if format == "json":
        ...
        with open(output_path, "w") as f:
            ...
            for r in suite.results:
                f.write(
                    f"{r.stage},{r.duration_ms:.2f},{r.p50_ms:.2f},{r.p95_ms:.2f},"
