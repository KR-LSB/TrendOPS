...
class YouTubeCollector(BaseCollector):
    ...
    BLOCK_INDICATORS = [
        ...
        "confirm you're not a robot", "sorry, something went wrong"
    ]
    ...
    async def fetch(self, keyword: str, **kwargs) -> List[TrendDocument]:
        ...
        try:
            ...
            logger.info(f"Fetching YouTube comments for '{keyword}' (Videos: {max_videos}, Comments/v: {comments_per_video})")
    ...
    async def fetch_comments(self, keyword: str, max_videos: int, comments_per_video: int) -> List[Comment]:
        """실제 수집 로직 (기존 코드 통합)"""
        all_comments = []
        ...
        try:
            ...
            for i, video in enumerate(videos):
                try:
                    ...
                    if i < len(videos) - 1:
                        await asyncio.sleep(random.uniform(self.config.video_delay_min, self.config.video_delay_max))
    ...
    async def _fetch_video_comments(self, video: VideoInfo, max_comments: int) -> List[Comment]:
        ...
        comments = []
        try:
            await page.goto(video.url, wait_until="domcontentloaded") # networkidle은 너무 느릴 수 있음
            await asyncio.sleep(2)
