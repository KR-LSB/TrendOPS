...
class RedisQueue:
    ...
    async def pop_job(self, timeout: int = 0) -> TrendJob | None:
        ...
        try:
            # BLPOP: 블로킹 팝 (FIFO)
            result = await self.client.blpop(RedisKeys.JOBS_PENDING, timeout=timeout)
    ...
    async def mark_processing(self, job_id: UUID | str) -> bool:
        ...
        try:
            ...
            logger.error(
                "Failed to mark job as processing", extra={"job_id": job_id_str, "error": str(e)}
            )
            return False
    ...
    async def mark_completed(self, job_id: UUID | str) -> bool:
        ...
        try:
            ...
            logger.error(
                "Failed to mark job as completed", extra={"job_id": job_id_str, "error": str(e)}
            )
            return False
    ...
    async def mark_failed(self, job_id: UUID | str, error_message: str) -> bool:
        ...
        try:
            ...
            logger.warning(
                "Job marked as failed", extra={"job_id": job_id_str, "error": error_message}
            )
            return True

        except RedisError as e:
            logger.error(
                "Failed to mark job as failed", extra={"job_id": job_id_str, "error": str(e)}
            )
            return False
